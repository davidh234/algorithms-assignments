...
public class CompetitionDijkstra {
	...
	/**
	 * @return int: minimum minutes that will pa ss before the three contestants can
	 *         meet
	 */
	public int timeRequiredforCompetition() {
		...
		for (int i = 0; i < Vertices.length; i++) {
			...
			if(shortestPaths != null) {
				for (int j = 0; j < shortestPaths.length; j++) {
					if (longestPath < shortestPaths[j] && shortestPaths[j] != Integer.MAX_VALUE) {
						longestPath = shortestPaths[j];
					} else if(shortestPaths[j] == Integer.MAX_VALUE) {
						return -1;
					}
				}
			} else {
		...
		return (int) Math.ceil((longestPath * 1000 / minTotal));
	}
	...
	private class Bag<Item> {
		...
		public String toString() {
			...
			if (head != null) {
				Edge current = head;
				while (current != null) {
					output = output + "Edge " + i + ", " + current.from + " -> " + current.to + ",  weight: "
							+ current.weight + "\n";
					current = current.next;
					i++;
				}
